...
3. **F final audit:** after confirmation/publication, inventory again because capture hooks may append history after D. Flush completion recording; batch remaining eligible history/log/index/report into one local develop-only audit commit, carried into the next release's D (no second develop push). Push only on explicit request, then verify finalDevelopSha remotely. Preserve releaseCandidateSha/tag: never promote audit to jenkins/jenkins-release, retag, amend candidate or bump. Inhouse also performs F.
4. **Final status:** verify tracked and untracked leftovers. Reconcile late eligible hook writes in the same authorized finalization; persistent writers mean pending, not an endless loop. Report unrelated/ignored/restricted exclusions with reasons. A local audit commit awaiting the next release is `carried-forward`, not a leftover. Failed audit push resumes F only; failed tag resumes tag then F.
